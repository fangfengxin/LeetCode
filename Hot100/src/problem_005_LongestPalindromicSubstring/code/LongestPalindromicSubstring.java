package problem_005_LongestPalindromicSubstring.code;
...
public class LongestPalindromicSubstring {
    ...
    public String longestPalindrome2(String s) {
        if (s == null) throw new NullPointerException();
        String res = "";
        String reverse = new StringBuffer(s).reverse().toString(); // 翻转字符串
        int length = s.length();
        int[][] arr = new int[length][length]; // 数组arr保存公共子串的长度
        int maxLen = 0; // 保存找到的最长回文字符串的长度
        int maxEnd; // 保存找到的最长回文字符串的尾部索引
    ...
    public String longestPalindrome3(String s) {
        ...
        String reverse = new StringBuffer(s).reverse().toString(); // 翻转字符串
        int length = s.length();
        int[] arr = new int[length]; // 数组arr保存公共子串的长度
        int maxLen = 0; // 保存找到的最长回文字符串的长度
        int maxEnd; // 保存找到的最长回文字符串的尾部索引
        for (int i = 0; i < length; i++) {
            for (int j = length - 1; j >= 0; j--) {
                ...
                if (s.charAt(i) == reverse.charAt(j)) {
                    ...
                    arr[j] = 0; // 使用二维数组的时候使用的是不同列，所以不用置零
                }
                // 更新最长回文子串信息
                // 当公共子串的尾部索引变换到原来字符串索引后，加上当前公共子串长度等于当前索引时，可以确定是回文子串
    ...
    public int expandCenter(String s, int left, int right) {
        ...
        return right - left - 1; // 减1是因为扩展结束比回文字符串实际长度多了2
    }
}